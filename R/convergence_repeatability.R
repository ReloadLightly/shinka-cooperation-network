# Step 3C: finite fixed-vector diagnostic replications, no refitting.
...
check_phase3_state <- function(z,x,fit,seed) {
  assert(x$nsub == 0L && x$n3 == 3000L && x$randomSeed == seed &&
...
run_cell <- function(root,output,label,seed,preflight=FALSE) {
  ...
  plan <- read_json("configs/convergence-repeatability-v1.json",simplifyVector=TRUE)
  assert(label %in% c("attempt3","attempt4"),"Unknown coefficient vector")
  ...
  old <- fit_diagnostics(fit,meta$original_draws)
  assert(identical(old$valid,meta$historically_accepted) && isTRUE(old$finite_identified), "Historical status changed")
  ...
  if(preflight) {
    # Source-proven native initialization; no simulator call or phase-3 body.
    dry <- stopped_native_initialization(fit,netdata,bound$effects,x)
    write_json(list(status="verified_no_simulation",fit=label,original=old,initialization=dry),
      file.path(output,"preflight.json"),auto_unbox=TRUE,pretty=TRUE,digits=17)
    return(list(status="verified_no_simulation",fit=label,new_simulations=0L))
  }
  x$randomSeed <- as.integer(seed)
  ...
  audit <- function(status) list(status=status,phase3_entries=st$phase3_entries,
    ...
    checked_every_simulator_call=TRUE,seed=seed,nsub=0L,n3=3000L,simOnly=FALSE,
  ...
  enter_phase <- function(z,x) {
    ...
    check_phase3_state(z,x,fit,seed);flush_audit("phase3_entered")
  }
  ...
  exit_sim <- function(z) {
    ...
    if(st$sim_exits %% 250L==0L) {
      flush_audit("running")
      cat(sprintf("STEP3C: %d/3000 draws; theta unchanged; elapsed %.1f s\n",st$sim_exits,proc.time()[[3]]-st$started))
      flush.console()
    }
  }
  before_postprocess <- function(z,x) {
    ...
    saveRDS(list(sf=z$sf,ssc=z$ssc,sf2=z$sf2,theta=z$theta,targets=z$targets,
      ...
      file.path(output,"raw-phase3.rds"),compress="xz")
  }
  ...
  fields <- c("theta","theta0","fixed","diver","newfixed","newFixed","requestedEffects",
    "targets","targets2","dfra","msf","covtheta","tconv","tconv.max","fchange",
    "OK","termination","Phase3Interrupt","Phase3nits","n","errorMessage.cov")
  saveRDS(result[intersect(fields,names(result))],file.path(output,"native-diagnostic.rds"),compress="xz")
  ...
  report<-list(status="completed",stage="3C",fit=label,seed=seed,draws=3000L,replications=1L,
    ...
    historical_attempt_accepted=meta$historically_accepted,new_refits=0L,new_forecasts=0L,target_packets_read=0L,
    raw_archives_read=0L,step4_started=FALSE,guard=audit("completed"),
    original_diagnostic=list(draws=meta$original_draws,maximum_absolute_t=old$maximum_absolute_t_ratio,overall=old$overall_maximum_convergence),
    ...
    interpretation="One declared repeatability cell. Prefix 1000 and full 3000 are nested, correlated samples; vectors use disjoint seed sets. Historical acceptance remains unchanged. This is conditional diagnostic variability, not fitting uncertainty, prediction quality, identification proof or search readiness.")
...
if(sys.nframe()==0L) {
  ...
  if(identical(args,"--self-test")) cat(toJSON(pilot_self_tests(),auto_unbox=TRUE),"\n")
    ...
    assert(length(args)==5L && args[1] %in% c("--execute-cell","--preflight-only"),"Expected --execute-cell or --preflight-only ROOT OUTPUT FIT SEED")
    report<-run_cell(normalizePath(args[2]),normalizePath(args[3]),args[4],as.integer(args[5]),args[1]=="--preflight-only")
